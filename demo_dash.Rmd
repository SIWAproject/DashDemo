...
```{r Prevalence Plot, echo=FALSE, include=TRUE, cache=TRUE, fig.height = 8, fig.width= 10}
...
pdiv + scale_color_cvi_d("alpha_colors")
```
...
plot <- ggplot(df, aes(
    ...
    geom_point(size=2) +xlab(p$labels$x) + ylab(p$labels$y) + scale_color_cvi_d("alpha_colors")
...
ggplot(df_agg,
       aes(x = Group,
           y = Abundance,
           fill = Genus,)) +  geom_bar(stat = "identity", position = "fill") + facet_wrap( ~Location, scales = 'free', nrow=1,ncol=2) + scale_fill_cvi_d("cvi_siwa") + theme(axis.text.x = element_text(angle = 60, hjust = 1)) + theme(strip.text.x = element_text(size = 20), text = element_text(size=20))
...
```{r ratios, include=FALSE, echo=FALSE}
folder <- "/Users/dvarelat/Documents/SIWA/Reports/Version1.0/Input_data/"
...
# ggplot(ratios_melt, aes(y = value, x =Group, fill = Group)) + 
#   ylab("Score 3") + xlab("") + 
#   geom_boxplot(outlier.shape = NA)  +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
#   geom_jitter(size=0.5)+
#   facet_grid(cols="variable") + scale_fill_cvi_d("groups_beta")
...
p <- ggplot(df, aes(x = log2FoldChange , y = reorder(taxa, -log2FoldChange), color=ProbioticScore)) + geom_point(size = 6) + 
  ...
  labs(fill = "taxa", color = "taxa", subtitle = c("Higher in Control", "Higher in Product"))  +
...
bacterias <- c("Lactobacillus", "Oscillospira","Bacteroides","Turicibacter","Streptococcus")
df <- data.frame(
...
df$names <-factor(df$names,levels = c("Lactobacillus", "Oscillospira","Bacteroides","Streptococcus","Turicibacter"))
