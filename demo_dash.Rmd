...
### Diversity comparison {data-width=300}
...
ggplot(df_agg,
       aes(x = Group,
           y = Abundance,
           fill = Genus,)) +  geom_bar(stat = "identity", position = "fill") + facet_wrap( ~Location, scales = 'free', nrow=1,ncol=2) + scale_fill_cvi_d("cvi_siwa") + theme(axis.text.x = element_text(angle = 60, hjust = 1)) + theme(strip.text.x = element_text(size = 13), text = element_text(size=13))
...
ggplot(data = subset,
            aes(x = labels, y = Abundance, fill = OTU)) + 
  geom_bar(stat = "summary",fun = "mean") +
  labs(title = "Taxonomic composition of some Genera (pathogens)") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  facet_grid(. ~ SampleLocation) + scale_fill_cvi_d("cvi_siwa")
...
### FIGURE 10.1: Deseq - Differentially abundant bacteria in the cecum (DESeq/LEfSe) {data-width=500}
```{r, include=TRUE, warning=FALSE, echo=FALSE}
...
### FIGURE 10.2: LEfSe - Differentially abundant bacteria in the cecum
{data-width=500}
```{r, include=TRUE, warning=FALSE, echo=FALSE}
bacterias <-
  c("Lactobacillus",
...
### FIGURE 11: Histopathology scores
...
ggplot(h, aes(y = mean ,
  ...
  geom_text(data = anno, aes(x = Group, y = ystar, label = lab))+
    ...
    theme(text = element_text(size = 12)) + facet_wrap(~Location, scales = 'free', nrow=1,ncol=2)  + scale_fill_cvi_d("cvi_siwa")
```
...
### FIGURE 12: One trait in the ilem. {data-width=600}
...
### FIGURE 13: Histopathology vs microbiome score. {data-width=600}
...
### FIGURE 14: Gene expression results. {data-width=400}
...
ggplot(g, aes(x =Group, y = neg_DD_IL10, fill = Group)) +
  ...
  geom_text(data = anno, aes(x = Group, y = ystar, label = lab))+
  stat_summary(fun=mean, geom="point", size=2, color="red", fill="red")  + facet_wrap(~Location, scales = "free_y", nrow=1, ncol=2) + scale_fill_cvi_d("groups")
...
### Connecting the microbiome with the genes. {data-width=400}
